...
if( (!isset($_POST["email"])) || ($to == "toemail@example.com" ) || ($replyemail == "youremail@example.com")|| ($to == "" )){
    ...
    if(1==1){
        ...
        $puntos_inspeccion =$_POST["puntos_inspeccion"];
        $trabajo_realizado =$_POST["trabajo_realizado"];



        $query = "INSERT INTO task(nit, direccion, telefono, entidad ) VALUES ('$nit', '$direccion', '$telefono', '$entidad')";
        ...
        $query = "SELECT * FROM task WHERE nit=$nit";
        $result_ = mysqli_query($conn, $query);
        if (mysqli_num_rows($result_) == 1) {
            $row = mysqli_fetch_array($result_);
            $task_id = $row['id'];
            $query = "INSERT INTO equipo(equipo,marca, modelo, serie, placa, task_id ) VALUES ('$equipo','$marca', '$modelo', '$serie', '$placa', '$task_id')";
            $result = mysqli_query($conn, $query);
            if (!$result){
                $_SESSION["message"] = "Query Failed";
                $_SESSION["message_type"] ="success";
        ...
        $_SESSION["message"] = "TAsk saved successfully:_".$entidad."--";
        $_SESSION["message_type"] ="success";   
    }
